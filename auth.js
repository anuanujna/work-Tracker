...
function canDemoLogin(username, password) {
  if (!username || !password) return false;
  return username === 'admin' && password === demoPassword();
}
...
loginForm.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    let response;
    if (staticDemo) {
      if (!canDemoLogin(username, password)) {
...
loginPasswordForm.addEventListener('submit', async (event) => {
  ...
  try {
    ...
    if (staticDemo) {
      ...
      response = await fetch('/api/change-password', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ username, newPassword }) });
...
document.getElementById('password-form').addEventListener('submit', async (event) => {
  ...
  const response = await fetch('/api/change-password', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ newPassword }) });
