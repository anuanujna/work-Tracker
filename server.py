...
def delete_report(report_id: str) -> bool:
    with connection() as database:
        cursor = database.execute("DELETE FROM reports WHERE id = ?", (report_id,))
...
class WorkTrackHandler(BaseHTTPRequestHandler):
    ...
    def do_DELETE(self) -> None:
        ...
        if path.startswith("/api/reports/"):
            report_id = path.rsplit("/", 1)[-1]
            if not report_id:
                self.send_json({"error": "Report ID is required"}, HTTPStatus.BAD_REQUEST)
                return
            if not delete_report(report_id):
                self.send_json({"error": "Report not found"}, HTTPStatus.NOT_FOUND)
                return
            audit(user[0], "report_deleted", f"Deleted report {report_id}", report_id)
            ...
            with connection() as database:
                ...
                if current_password:
                    if not password_matches(current_password, account["password_hash"]):
                        ...
                        return
                elif authenticated_user and authenticated_user[0] != username:
                    self.send_json({"error": "You can only change your own password"}, HTTPStatus.FORBIDDEN)
                    return
