...
function renderReportLibrary() {
  ...
  reportList.innerHTML = viewedReports.length ? viewedReports.map((report, index) => {
    ...
    return `<div class="report-row"><div class="report-file-icon">▤</div><div class="report-details"><strong>${report.name}</strong><span>${report.rowCount} activities · Viewed ${date}</span></div><div class="report-actions"><button class="delete-report" data-report-id="${report.id}" type="button" title="Delete report" aria-label="Delete ${report.name}">🗑</button><button class="text-button reopen-report" data-report-index="${index}" type="button">Open report →</button></div></div>`;
  ...
  reportList.querySelectorAll('.delete-report').forEach((button) => button.addEventListener('click', async () => {
    const report = viewedReports.find((item) => item.id === button.dataset.reportId);
    if (!report) return;
    const confirmed = window.confirm(`Delete "${report.name}" from the report library?`);
    if (!confirmed) return;
    try {
      const response = await fetch(`/api/reports/${encodeURIComponent(report.id)}`, { method: 'DELETE' });
      if (!response.ok) {
        const errorBody = await response.json().catch(() => ({}));
        throw new Error(errorBody.error || 'Could not delete this report.');
      }
      viewedReports = viewedReports.filter((item) => item.id !== report.id);
      renderReportLibrary();
      showToast(`${report.name} deleted.`);
    } catch (error) {
      showToast(error.message || 'Could not delete this report.');
    }
  }));
}
